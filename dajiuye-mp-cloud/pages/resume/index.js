...
Page({
    ...
    onShow: function () {
        if (this.data.isLogin) {
            ...
            // console.log("time")
            // console.log(resumeUrl.substring(resumeUrl.lastIndexOf('_')+1,resumeUrl.lastIndexOf('-')))
            if (resumeUrl) {
                this.setData({
                    resumeUrl,
    ...
    async setUserResume(openId) {   
        const result = await request({ url: "/user/getUser", data:{openId}});
    ...
    touchphoto(e) {
        ...
        wx.chooseMessageFile({
            ...
            success: function (res) {
                ...
                // {errMsg: "chooseMessageFile:ok", tempFiles: Array(1)}
                // errMsg: "chooseMessageFile:ok"
                // tempFiles: Array(1)
                // 0:
                // name: "0.png"
                // path: "http://tmp/TeU6C18sJg3I14e59eff3cd379c6de93262396b9249c.png"
                // size: 97707
                // time: 1635231118
                // type: "image"
                console.log(res)
...
function formSubmit() {
    ...
    httputil.send_photo(photo, function (res) {
        ...
        // 更新简历的详细信息（resumeUrl，resumeTime，resumeName）
        getCurrentPages()[getCurrentPages().length - 1].setUserResume(getCurrentPages()[getCurrentPages().length - 1].data.userInfo.myEncrypt)
    })
}